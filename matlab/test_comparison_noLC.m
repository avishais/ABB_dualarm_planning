...
% PCS on one scene with obstacles where motion toward a random
% configuration is performed as much as possible with small (resolution
% size) steps.
...
%%
D1 = load('benchmark_PCS_3poles_noLC.txt'); 
verf = D1(:,1)==1;
suc = D1(:,2)==1;
...
disp(['Avg. number of steps toward a target conf.: ' num2str((mean(D1(:,13)))) ]);



%%
D2 = load('benchmark_GD_3poles_noLC.txt'); 
verf = D2(:,1)==1;
suc = D2(:,2)==1;
...
disp(['Avg. number of steps toward a target conf.: ' num2str((mean(D2(:,13)))) ]);

%%
% PCS
td = D1(:,4);
maxT = max(td);
T1 = linspace(0,maxT,400);
T1 = T1(2:end);
for i = 1:length(T1)
    sd = td < T1(i);
    md(i) = mean(td(sd));
